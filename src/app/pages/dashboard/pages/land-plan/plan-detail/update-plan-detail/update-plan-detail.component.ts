...
import {
  CreatePlanDetailDto,
  DescriptionReadParameterPlanDetailDto,
  ReadPlanDetailDto,
} from 'src/app/common/plan-detail.model';
import {
  ...
  UpdateParameter
} from 'src/app/common/plan-detailparameter.model';
import { PlanDetailService } from 'src/app/api-services/plan-detail.service';
import { filter, switchMap, tap } from 'rxjs';
import { NzNotificationModule, NzNotificationService } from 'ng-zorro-antd/notification';
import { PlanDetailParameterService } from 'src/app/api-services/plan-detail-parameter.service';
import {NzSelectModule} from "ng-zorro-antd/select";
import {Role} from "../../../../../../common/account.model";
import {IfRolesDirective} from "../../../../../../directives/if-roles.directive";

@Component({
  selector: 'app-update-plan-detail',
  standalone: true,
  templateUrl: './update-plan-detail.component.html',
  styleUrls: ['./update-plan-detail.component.scss'],
  imports: [
    CommonModule,
    IfRolesDirective,
    ReactiveFormsModule,
...
export class UpdatePlanDetailComponent implements OnInit {
  @Input() data!: ReadPlanDetailDto;
  ...
  constructor(
    ...
    private PlanDetailParameterService: PlanDetailParameterService,
  ) {}

  ngOnInit(): void {
    this.insertDataToForm();
    this.PlanDetailParameterService.showAllParam()
  ...
  insertDataDescriptionParameterToForm(
    data: DescriptionReadParameterPlanDetailDto[]
  ): FormArray {
    const temp: FormArray = this.fb.nonNullable.array([]);
  ...
  updatePlanDetail():void{
    console.log(this.form.valid, this.form.value);

    if(this.form.valid){
      this.PlanDetailService.update(this.form.controls['Id'].value, {
  ...
  saveParameter(idx_parameter:number):void{
    ...
    if(isAllNew){
      console.log("Buat group PlanDetail parameter baru");
      ...
      const obj:CreateParameter={
        PlanDetailId: this.data.Id,
        ParentTypeId:param.get('ParentTypeId')?.value,
        Descriptions:descs
      }
      console.log(obj);
      this.PlanDetailParameterService.createGroup(obj)
      .pipe(
      ...
      console.log(obj);
      this.PlanDetailParameterService.updateGroup(this.data.Id,obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
  ...
  saveDescriptionParameter(idx_parameter:number,idx_desc_parameter: number):void{
    ...
    if(descParam.get('Id')?.value !== 0){
      ...
      console.log(obj);
      this.PlanDetailParameterService.update(descParam.get('Id')?.value,obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
      ...
      console.log(obj);
      this.PlanDetailParameterService.create(obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
  ...
  deleteParameter(i:number):void{
    ...
    .pipe(
      ...
      filter(x=>x!==-1),
      switchMap(x=>{
          return this.PlanDetailParameterService.deleteGroup(this.data.Id, x);
      }),
      tap(()=>this.notification.create(
        'success',
        'Sukses',
  ...
  deleteDescParameter(i:number, j:number):void{
    ...
    .pipe(
      filter(x=>x!==-1),
      switchMap(x=>this.PlanDetailParameterService.delete(x)),
  ...
  deletePlanDetail():void{
    ...
    .pipe(
      filter(x=>x!==-1),
      switchMap(x=>{
          return this.PlanDetailService.delete(x);
      }),
      tap(()=>this.notification.create(
        'success',
        'Sukses',
