...
import {
  CreatePlanDto,
  DescriptionReadParameterPlanDto,
  ReadPlanDto,
} from 'src/app/common/plan.model';
import {
  ...
  UpdateParameter
} from 'src/app/common/planparameter.model';
import { PlanService } from 'src/app/api-services/plan.service';
import { filter, switchMap, tap } from 'rxjs';
import { NzNotificationModule, NzNotificationService } from 'ng-zorro-antd/notification';
import { PlanParameterService } from 'src/app/api-services/plan-parameter.service';
...
@Component({
  ...
  standalone: true,
  templateUrl: './update-land-plan.component.html',
  styleUrls: ['./update-land-plan.component.scss'],
  imports: [
    CommonModule,
    IfRolesDirective,
    ReactiveFormsModule,
...
export class UpdatePlanComponent implements OnInit {
  ...
  insertDataDescriptionParameterToForm(
    data: DescriptionReadParameterPlanDto[]
  ): FormArray {
    const temp: FormArray = this.fb.nonNullable.array([]);
  ...
  updatePlan():void{
    ...
    if(this.form.valid){
      this.PlanService.update(this.form.controls['Id'].value, {
  ...
  saveParameter(idx_parameter:number):void{
    ...
    if(isAllNew){
      console.log("Buat group Plan parameter baru");
      ...
      console.log(obj);
      this.PlanParameterService.createGroup(obj)
      .pipe(
      ...
      console.log(obj);
      this.PlanParameterService.updateGroup(this.data.Id,obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
  ...
  saveDescriptionParameter(idx_parameter:number,idx_desc_parameter: number):void{
    ...
    if(descParam.get('Id')?.value !== 0){
      ...
      console.log(obj);
      this.PlanParameterService.update(descParam.get('Id')?.value,obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
      ...
      console.log(obj);
      this.PlanParameterService.create(obj)
      .pipe(
        tap(()=>this.notification.create(
          'success',
          'Sukses',
  ...
  deleteParameter(i:number):void{
    ...
    .pipe(
      ...
      filter(x=>x!==-1),
      switchMap(x=>{
          return this.PlanParameterService.deleteGroup(this.data.Id, x);
      }),
      tap(()=>this.notification.create(
        'success',
        'Sukses',
  ...
  deleteDescParameter(i:number, j:number):void{
    ...
    .pipe(
      filter(x=>x!==-1),
      switchMap(x=>this.PlanParameterService.delete(x)),
  ...
  deletePlan():void{
    ...
    .pipe(
      filter(x=>x!==-1),
      switchMap(x=>{
          return this.PlanService.delete(x);
      }),
      tap(()=>this.notification.create(
        'success',
        'Sukses',
